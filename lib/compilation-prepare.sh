...
compilation_prepare()
{
	...
	if linux-version compare $version ge 5.6 && [[ "$BRANCH" == current || "$BRANCH" == dev ]] && [[ "$BOARDFAMILY" != "rk3328-tv" ]] && [[ "$BOARDFAMILY" != "rk3399-tv" ]] && [[ "$BOARDFAMILY" != "aml-s9xxx" ]] && [[ "$BOARDFAMILY" != "arm-64" ]] && [[ "$BOARDFAMILY" != "aw-h6-tv" ]] && [[ "$BOARDFAMILY" != "aml-s812" ]]; then

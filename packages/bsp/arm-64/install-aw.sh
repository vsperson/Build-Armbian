...
dd if="${DEV_EMMC}" of=/root/u-boot-default-aw.img bs=1M count=16

dd if=/dev/zero of="${DEV_EMMC}" bs=512 count=1

dd if=/root/u-boot-default-aw.img of="${DEV_EMMC}" bs=1 count=442
dd if=/root/u-boot-default-aw.img of="${DEV_EMMC}" bs=512 skip=1 seek=1
...
if [ -f /root/u-boot/u-boot-aw/uboot.img ] ; then
    ...
    dd if=/root/u-boot/u-boot-aw/uboot.img of="${DEV_EMMC}" conv=fsync bs=1k seek=8
fi
